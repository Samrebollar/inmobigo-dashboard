...
async function run() {
  const { data: d5, error: e5 } = await supabase
    ...
    .select('id, profiles(full_name, email, residents(units(unit_number)))')
    .limit(1)

  if (e5) {
      console.log('Error deeper join', e5.message)
  } else {
      console.log('Success deeper join', JSON.stringify(d5, null, 2))
  }
}
run()
