
import { createClient } from '@supabase/supabase-js'

const supabaseUrl = 'https://djxllvplxdigosbhhicn.supabase.co'
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRqeGxsdnBseGRpZ29zYmhoaWNuIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MDIzODMwMywiZXhwIjoyMDg1ODE0MzAzfQ.W8sB7_M_2smipPtbXcADjArL1s_tCigha09h3S8Xrzg'
const supabase = createClient(supabaseUrl, supabaseKey)

async function getColumns() {
    console.log("--- Getting Columns for 'incidents' ---")
    // Note: PostgREST doesn't support querying information_schema directly via .from()
    // unless we have a view or RPC.
    // But we can try to insert an empty object and see the error message which often lists missing columns.
    
    const { error } = await supabase.from('incidents').insert({}).select()
    if (error) {
        console.log("Error (expected):", error.message)
        // Sometimes the error message contains hints about columns
    }
}

getColumns()
