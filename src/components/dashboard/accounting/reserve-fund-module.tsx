...
export function ReserveFundModule({ fundData, condominiumId, isAdmin = false }: ReserveFundModuleProps) {
    ...
    return (
        <div className="space-y-8 py-4">
            ...
            {/* El Fondo de Reserva se administra por condominio — sin uno
                seleccionado no hay saldo/meta/aportación que mostrar, así que
                se explica esto en vez de mostrar tarjetas en $0 que parecen
                datos vacíos por error. */}
            {condominiumId === 'all' ? (
                <div className="p-6 rounded-[2.5rem] bg-indigo-500/5 border border-indigo-500/10 flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        <p className="text-sm font-bold text-indigo-200/90">El Fondo de Reserva se administra por condominio.</p>
                        <p className="text-xs text-zinc-500 mt-0.5">Selecciona una propiedad específica arriba para ver su saldo, meta recomendada, progreso y aportación mensual configurada.</p>
                    </div>
                </div>
            ) : (
                <>
