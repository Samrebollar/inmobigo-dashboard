...
export default async function PasePage({ params }: { params: Promise<{ code: string }> }) {
    ...
    // Vigencia real basada en la fecha/hora de la visita (offset fijo de México, UTC-6),
    // igual que la página de check-in en /[id] — no un límite arbitrario de 24h.
    const now = new Date()
    const visitDateTime = new Date(`${pass.visit_date}T${pass.end_time || '23:59:59'}-06:00`)
    const isExpired = pass.status === 'pending' && now > visitDateTime

    if (isExpired || pass.status !== 'pending') {
        const titleLine = isExpired ? 'Pase Expirado' : pass.status === 'cancelled' ? 'Pase Cancelado' : 'Pase Utilizado'
                ...
                <Card className="bg-zinc-900/80 backdrop-blur-xl border-zinc-800 max-w-sm w-full mx-auto shadow-2xl relative z-10 overflow-hidden rounded-[2rem]">
                    <div className={`${bgClass} p-8 flex flex-col items-center border-b ${borderClass}`}>
                        ...
                        <p className="text-zinc-400 text-center text-sm">
                            {isExpired
                                ? 'El tiempo de validez de este pase ha terminado.'
                                : pass.status === 'cancelled'
                                    ? 'Este pase fue anulado por el residente.'
                                    : 'Este pase ya fue validado anteriormente y es de un solo uso.'}
                        </p>
                    </div>
                     <CardContent className="p-6 space-y-6">
                         <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div><p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider mb-1">Visitante</p><p className="font-semibold text-white">{pass.visitor_name}</p></div>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                        ...
                        <div className="p-6 bg-black/40 rounded-3xl border border-zinc-800/80 space-y-6 shadow-inner">
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-white capitalize">{pass.visitor_name}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-lg font-bold text-white tracking-tight">{pass.unit_name || 'S/D'}</p>
                                    <p className="text-xs font-bold text-zinc-400 mt-1">Autoriza: <span className="text-zinc-300">{pass.authorized_by_name || 'Residente'}</span></p>
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mb-1">Vigencia</p>
                                    <p className="text-sm font-bold text-white capitalize">{format(new Date(`${pass.visit_date}T00:00:00`), "EEEE d, MMM", { locale: es })}</p>
                                    <p className="text-xs font-medium text-amber-500/80 mt-1">
                                        {pass.start_time?.slice(0, 5)}{pass.end_time ? ` - ${pass.end_time.slice(0, 5)}` : ''} hs · Pase de un solo uso.
                                    </p>
                                </div>
                            </div>
                        </div>
