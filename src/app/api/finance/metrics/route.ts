...
export async function GET(request: Request) {
    try {
        ...
        // YYYY-MM-DD format for due_date querying
        const yyyy = currentYear
        const mm = String(currentMonth + 1).padStart(2, '0')
        const lastDay = new Date(currentYear, currentMonth + 1, 0).getDate()
        const startDateStr = `${yyyy}-${mm}-01`
        const endDateStr = `${yyyy}-${mm}-${String(lastDay).padStart(2, '0')}`

        let currentMonthInvoices: any[] = []
        try {
            let invQuery = supabase
                .from('resident_invoices')
                .select('amount, balance_due, status, due_date, created_at, invoice_type')
            ...
            if (!invsError && invs) {
                currentMonthInvoices = invs
            }
        } catch (e) {
            console.error('Error fetching current month invoices:', e)
        }
        ...
        // ─────────────────────────────────────────────────────────────────────
        // 2, 3 & 4. TOTAL POR COBRAR, CARTERA VENCIDA & EFICACIA DE COBRO
        // ─────────────────────────────────────────────────────────────────────
        let total_por_cobrar = 0
        let cartera_vencida = 0
        let total_generado = 0
        let recaudado_mes_inv = 0
        let hasMonthlyData = false

        if (currentMonthInvoices.length > 0) {
            const maintenanceInvoices = currentMonthInvoices.filter(
                (inv) => inv.invoice_type === 'maintenance'
            )
            const invoicesToUse = maintenanceInvoices.length > 0 ? maintenanceInvoices : currentMonthInvoices

            total_por_cobrar = invoicesToUse.reduce(
                (sum, inv) => sum + Number(inv.balance_due || 0),
                0
            )

            const isPastDay10 = now.getDate() > 10
            cartera_vencida = invoicesToUse.reduce(
                (sum, inv) => {
                    const effectiveStatus = (inv.status === 'pending' && isPastDay10) ? 'overdue' : inv.status
            ...
            total_generado = invoicesToUse.reduce(
        ...
        const eficacia_cobro = hasMonthlyData
            ? (total_generado > 0 ? Math.min(100, Math.max(0, Math.round((recaudado_mes_inv / total_generado) * 10000) / 100)) : 0)
            : (total_generado > 0 ? Math.min(100, Math.max(0, Math.round((ingresos_mes / total_generado) * 10000) / 100)) : 0)
