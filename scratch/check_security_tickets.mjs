...
async function checkSecurityTickets() {
    const { data, error } = await supabase.from('tickets').select('*').eq('category', 'security')
    console.log("Security Tickets found:", data?.length || 0)
    if (data && data.length > 0) {
        console.log("Sample:", data[0])
    }
}

checkSecurityTickets()
