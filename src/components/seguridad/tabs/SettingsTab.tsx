...
import { Save, Zap, Plus, Trash2, FileText, Upload, ExternalLink } from 'lucide-react'
...
export function SettingsTab() {
    ...
    const handleReglamentoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const filePath = `${condominiumId}/reglamento-${Date.now()}.pdf`
        ...
        try {
            ...
            const updated = await propertiesService.update(condominiumId, {
                reglamento_url: data.publicUrl,
                reglamento_uploaded_at: uploadedAt
            })
            setCondo(updated)
    ...
    const handleRemoveReglamento = async () => {
        if (!confirm('¿Seguro que deseas eliminar el reglamento subido? Los residentes ya no podrán consultarlo.')) return
        try {
            const updated = await propertiesService.update(condominiumId, {
                reglamento_url: null,
                reglamento_uploaded_at: null
            })
            setCondo(updated)
    ...
    return (
        <div className="space-y-6 max-w-4xl">
            ...
            <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader>
                    <CardTitle>Reglamento del Condominio</CardTitle>
                    <CardDescription>Sube el reglamento oficial en PDF para que quede disponible en la plataforma.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    {condo.reglamento_url ? (
                        <div className="flex items-center justify-between p-4 rounded-xl bg-zinc-950/50 border border-zinc-800">
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <p className="font-medium text-white truncate">Reglamento_{isPropiedades ? 'Propiedad' : 'Condominio'}.pdf</p>
                                    {condo.reglamento_uploaded_at && (
                                        <p className="text-xs text-zinc-500">
                                            Subido el {new Date(condo.reglamento_uploaded_at).toLocaleDateString('es-MX', { day: 'numeric', month: 'long', year: 'numeric' })}
                                        </p>
                                    )}
                                </div>
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                                <a href={condo.reglamento_url} target="_blank" rel="noopener noreferrer">
                                ...
                                <Button type="button" variant="outline" onClick={handleRemoveReglamento} className="border-rose-500/30 bg-rose-500/10 text-rose-400 hover:bg-rose-500/20 hover:text-rose-300">
                        ...
                        <label className="flex flex-col items-center justify-center gap-3 p-8 rounded-xl border border-dashed border-zinc-800 bg-zinc-950/30 hover:border-indigo-500/40 hover:bg-zinc-900/40 transition-colors cursor-pointer">
                            <input type="file" accept="application/pdf" className="hidden" onChange={handleReglamentoUpload} disabled={uploadingReglamento} />
